...
Cập nhật thông tin phim (tương tự POST nhưng tất cả đều là optional).
...
| `seatType` | string | ❌ | Loại ghế mặc định (`Thường`, `VIP`, `Đôi`) |
...
Cập nhật ghế. Các trường tương tự POST nhưng đều là optional.
...
Xem tất cả booking của hệ thống (Admin xem toàn bộ, Staff chỉ xem rạp của mình).
...
Sửa đánh giá của mình. Chỉ được sửa đánh giá do chính mình tạo.

### `DELETE /reviews/:id` [AUTH]

Xóa đánh giá. User xóa của mình, Admin/Staff có thể xóa bất kỳ.
