...
export class BookingsService {
    ...
    const result = await Promise.all(
      seats.map(async (seat) => {
        ...
        if (soldSeatIds.has(seat.seatId)) {
          ...
          const isHeld = await this.cacheManager.get(`hold:${showtimeId}:${seat.seatId}`);
          if (isHeld) {
            status = 'HELD';
          }
        }
  ...
  async createBooking(username: string, data: CreateBookingDto, ipAddr: string) {
    const showtime = await this.prisma.showtime.findUnique({
    ...
    if (soldSeats.length > 0) {
      throw new ConflictException('Một số ghế bạn chọn đã có người thanh toán thành công!');
    }

    // truy vấn redis xem có ghế nào đang bị khóa tạm thời bởi người khác không
    for (const seatId of data.seats) {
      const heldBy = await this.cacheManager.get(`hold:${data.showtimeId}:${seatId}`);
      if (heldBy && heldBy !== username) {
        throw new ConflictException('Ghế bạn chọn đang được khách hàng khác giữ để thanh toán. Vui lòng chọn ghế khác hoặc thử lại sau 5 phút.');
      }
    }
    ...
    if (data.foods && data.foods.length > 0) {
      for (const food of data.foods) {
        ...
        if (!foodItem) throw new NotFoundException(`Món ăn ${food.foodId} không tồn tại`);
    ...
    for (const seatId of data.seats) {
      await this.cacheManager.set(`hold:${data.showtimeId}:${seatId}`, username, 300000);
    }
    ...
    const payment = await this.paymentsService.createPaymentUrl(booking.bookingId, ipAddr);
