...
class PandasRecommendationService:
    ...
    def process_logic(self, user_id="system"):
        ...
        for user in users:
            ...
            # Bước 1: Gom nhóm và Thống kê tần suất (Frequency Analysis)
            seen_movie_ids = set(user_hist["movieId"].tolist())
            
            # Lọc ra những phim có đánh giá >= 3 hoặc chưa đánh giá để làm căn cứ tìm Gu phim
            # Bỏ qua những phim bị đánh giá thấp (< 3)
            ...
            # Phân tích Đạo diễn / diễn viên yêu thích
            ...
            # Bỏ qua nếu không có phim nào đang chiếu
            if df_active_movies.empty:
                continue

            # Lọc ra danh sách phim chưa xem
            df_unseen = df_active_movies[~df_active_movies["_id"].isin(seen_movie_ids)].copy()
            
            if df_unseen.empty:
                continue

            # Bước 2: Chấm điểm Phim đang chiếu (Weighted Scoring)
            def calculate_score(row):
                ...
                # Trọng số Con người (Diễn viên/Đạo diễn)
                people = []
